    ...
    def form_valid(self, form):
        ...
        if user.email:
            try:
                ...
                messages.info(self.request, f"✉️ A 6-digit verification code has been sent to {user.email}.")
            except Exception as e:
                import logging
...
class EmailOTPVerifyView(LoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if action == 'resend':
            ...
            if recent_resends >= 3:
                messages.error(request, "⚠️ Maximum resend attempts reached. Please wait 15 minutes before requesting another OTP.")
        ...
        if timezone.now() > otp_record.expires_at:
            messages.error(request, "⚠️ Verification code has expired. Please click Resend OTP.")
            return render(request, self.template_name, {'user': user, 'expired': True})

        if otp_record.attempts_count >= 5:
            messages.error(request, "⚠️ Too many incorrect attempts. Please click Resend OTP for a new code.")
            return render(request, self.template_name, {'user': user})
...
class VehicleDeleteView(LoginRequiredMixin, View):
    def post(self, request, pk, *args, **kwargs):
        ...
        vehicle.delete()
        messages.success(request, f"🗑️ Vehicle '{make_name}' has been deleted.")
        return redirect('accounts:profile')
    ...
    def post(self, request, *args, **kwargs):
        ...
        if dob_str:
            try:
                ...
                if age < 18:
                    messages.error(request, "⚠️ You must be at least 18 years old to use Kaapool.")
